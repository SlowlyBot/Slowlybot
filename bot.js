...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("Vatan Roleplay Server Status", "https://cdn.discordapp.com/attachments/726534671097135208/726559880579776563/logodc.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Vatan Roleplay' : '⚫ Vatan Roleplay')
    .setTimestamp(new Date())
    .addField('\n\u200b\nSunucuya nasıl katılabilirsiniz?','Sunucuya aşağıdakileri kullanarak katılabilirsiniz: ** Launcher ** Tikla Ve indir  Launcher rardan çikar masaustune at Vatanrp ye tikla launcher açilsin il ts3 tikla teamspeak3 açilir sonra oyna tuşuna tikla ve oyuna giriyor.  En altta, kaç kişinin çevrimiçi ve kuyrukta olduğunu görmek için sunucu durumu bulunur. [Launcher](https://cdn.discordapp.com/attachments/726560102668304526/729527322893353000/VatanRP.rar/)\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const offline = function() {
    ...
    .addField('Sirada Bekleyenler','?',true)
    .addField('Online Oyuncu','?\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('Roofstad', 'https://www.twitch.tv/JamshidTR');
   // bot.user.setActivity('Vatan Roleplay',{'url':'https://www.twitch.tv/JamshidTR','type':'STREAMING'});
      ...
      if (message.member) {
        if (message.member.hasPermission('ADMINISTRATOR')) {
          if (message.content.startsWith('+status ')) {
            ...
            .setColor(0x2894C2)
            .setTitle('Güncellenmiş durum mesajı')
            .setTimestamp(new Date());
            if (status === 'clear') {
              STATUS = undefined;
              embed.setDescription('Silinmiş durum mesajı');
            } else {
              STATUS = status;
        ...
        if (message.channel.id === SUGGESTION_CHANNEL) {
          ...
          message.channel.send(embed).then((message) => {
            ...
            sent.react('👍').then(() => {
              sent.react('👎').then(() => {
                log(LOG_LEVELS.SPAM,'Tamamlanan öneri mesajı');
              }).catch(console.error);
            }).catch(console.error);
          }).catch(console.error);
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setDescription('Mesajınız başarıyla personel ekibine gönderildi!')
          .setTimestamp(new Date());
  ...
  bot.login(BOT_TOKEN).then(null).catch(() => {
    log(LOG_LEVELS.ERROR,'Giriş yapılamıyor Giriş jetonunuzu kontrol edin');
    console.error(e);
    process.exit(1);
  });
